...
public class AxcRowAverageFilter: AxcBaseReductionFilter,
    ...
    override var axc_ciImage: CIImage? {
        ...
        let rect = CGRect(x: CGFloat(0), y: CGFloat(0),
                          width: extent.size.axc_biggerValue, height: extent.size.axc_biggerValue)
        guard let cgImage = context.createCGImage(ciImage, from: rect)?.axc_rotate(angle: 90) else { return nil }
        var draw_y = extent.size.axc_biggerValue - extent.size.axc_smallerValue
        if draw_y < 0 { draw_y = 0 }
        let drawRect = CGRect(x: rect.width - 1, y: 0, width: 1, height: rect.height - draw_y)
        guard let drawCGImage = context.createCGImage(cgImage.axc_ciImage, from: drawRect) else { return nil }

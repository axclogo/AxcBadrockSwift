...
public extension URL {
    ...
    var axc_allParam: [String: String]? {
        guard let components = axc_urlComponents, let queryItems = components.queryItems else { return nil }
        var _param: [String: String] = [:]
        for queryItem in queryItems { _param[queryItem.name] = queryItem.value }
        return _param
    }

...
private var ktextDidChange = UITextField.textDidChangeNotification
public extension UITextField {
    typealias AxcUITextFieldBlock = (UITextField) -> Void
    /// 文本发生变化的回调
    var axc_textChangeBlock: AxcUITextFieldBlock? {
        set { AxcRuntime.setObj(self, &kaxc_textChangeBlock, newValue)
            axc_removeNotification(ktextDidChange) // 先移除
            axc_addNotification(ktextDidChange, selector: #selector(textChange))
        }
        get { guard let block = AxcRuntime.getObj(self, &kaxc_textChangeBlock) as? AxcUITextFieldBlock else { return nil }
            return block
        }
    }
    /// 文本发生变化的触发
